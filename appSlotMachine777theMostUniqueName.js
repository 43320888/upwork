...
function setSize() {
	...
	switch (Boolean(config.tableHeight)) {
		case true:
			if (config.tableHeight.includes('%')) sizes.wrapper.h = sizes.wrapper.w * `0.${Number.parseInt(config.tableHeight)}`;
...
const alertPrize = () => {
	...
	if (config.postType === 'new page') {
		...
		modalWindow.style.cssText = `
				...
				background: #fff8;
			`;
		modalWindow.src = `${config.contactForm}?winningSegment=${winningSegment.text}`;
