(() => {
	...
	function setSize() {
		...
		switch (Boolean(config.tableHeight)) {
			case true:
				size.wrap.h = config.tableHeight;
		...
		/* TITLE */
		e.h.style.top = `${size.wrap.h / 20}px`;
		e.h.style.width = `${size.wrap.w}px`;
		e.h.style.height = `${size.wrap.h / 10}px`;
		/* top, left, width, height */

		/* CANVASWRAPPER */
		/* top, left, width, height */
		if (size.wrap.w > size.wrap.h) {
			e.canvaswrapper.style.top = `${size.wrap.h * 0.25}px`;
			e.canvaswrapper.style.width = `${size.wrap.h * 0.7}px`;
			e.canvaswrapper.style.left = `${size.wrap.w / 2 - size.wrap.h * 0.7 / 2}px`;
		} else {
			e.canvaswrapper.style.top = `${size.wrap.h / 2 * size.wrap.w * 0.5 / 2}px`;
			e.canvaswrapper.style.width = `${size.wrap.w * 0.7}px`;
			e.canvaswrapper.style.top = `${size.wrap.h / 2 - size.wrap.w * 0.7 / 2}px`;
		}
		// max-with 777px
