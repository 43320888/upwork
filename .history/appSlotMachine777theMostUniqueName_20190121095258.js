(() => {
	...
	e.style.innerHTML = `
		...
		.appSlotMachine777theMostUniqueName {
			...
			/* background-image: url(${config.pictures.wrapper}); */
		...
		.appSlotMachine777theMostUniqueName div:first-child {
			...
			/* background-image: url(${config.pictures.title}); */
			/* background-image: url(koleso8.jpg); */
			background-position-x: center;
		...
		.appSlotMachine777theMostUniqueName div:nth-child(3) {
			...
			background: white;
			/* background-image: url(); */
			/* background-image: url(${config.pictures.pin}); */
			/* background-image: url(koleso8.jpg); */
			/* background-size: contain; */
			z-index: 2;
		}
		...
		.appSlotMachine777theMostUniqueName #canvasSlotMachine777theMostUniqueName {
			...
			background: #ec8;
			background-size: contain;
			/* background-image: url(${config.pictures.table}); */
		...
		.appSlotMachine777theMostUniqueName div:nth-child(4) {
			...
			background: white;
			/* background-size: contain; */
			/* background-image: url(${config.pictures.spin}); */
			/* background-image: url(koleso8.jpg); */
		}
	...
	function setSize() {
		...
		// /* WRAPPER */
		size.wrapper.w = e.wrap.clientWidth;
		switch (Boolean(config.tableHeight)) {
			case true:
				size.wrapper.h = config.tableHeight;
				e.wrap.style.height = `${config.tableHeight}px`;
				break;
			default:
				if (size.wrapper.w <= document.documentElement.clientHeight || size.wrapper.w <= window.innerHeight) {
					size.wrapper.h = window.innerHeight;
					e.wrap.style.height = `${size.wrapper.h}px`;
				} else {
					size.wrapper.h = window.innerHeight;
					e.wrap.style.height = `${size.wrapper.h}px`;
				}
		...
		/* TITLE */
		e.h.style.top = `${size.wrapper.h / 20}px`;
		e.h.style.width = `${size.wrapper.w}px`;
		e.h.style.height = `${size.wrapper.h / 10}px`;
		/* top, left, width, height */

		/* CANVASWRAPPER */
		/* top, left, width, height */
		if (size.wrapper.w > size.wrapper.h) {
			e.canvaswrapper.style.top = `${size.wrapper.h * 0.25}px`;
			e.canvaswrapper.style.width = `${size.wrapper.h * 0.7}px`;
			e.canvaswrapper.style.left = `${size.wrapper.w / 2 - size.wrapper.h * 0.7 / 2}px`;
		} else {
			size.canvasWrapper = size.wrapper.w * 0.7;
			e.canvaswrapper.style.width = `${size.canvasWrapper}px`;
			e.canvaswrapper.style.top = `${size.wrapper.h / 2 - size.canvasWrapper / 2}px`;
			e.canvaswrapper.style.top = `${size.wrapper.h / 2 - size.wrapper.w * 0.7 / 2}px`;
		}
		// max-with 777px
