(() => {
	...
	function setSize() {
		...
		/* CANVASWRAPPER */
		/* top, left, width, height */
		if (sizes.wrapper.w > sizes.wrapper.h || sizes.wrapper.w > 650/*  (sizes.wrapper.w > 650 && Math.abs(sizes.wrapper.w - sizes.wrapper.h) > 55) */) {
			e.canvaswrapper.style.top = `${sizes.wrapper.h * 0.25}px`;
			e.canvaswrapper.style.width = `${sizes.wrapper.h * 0.7}px`;
			e.canvaswrapper.style.left = `${sizes.wrapper.w / 2 - sizes.wrapper.h * 0.7 / 2}px`;
		} else {
			sizes.canvasWrapper = sizes.wrapper.w * 0.9;
			e.canvaswrapper.style.width = `${sizes.canvasWrapper}px`;
			// debugger;
			e.canvaswrapper.style.top = `${sizes.wrapper.h * 0.1 + (sizes.wrapper.h / 2 - sizes.canvasWrapper / 2) }px`;
			e.canvaswrapper.style.left = `${sizes.wrapper.w / 2 - sizes.canvasWrapper / 2}px`;
		}
		// max-with 777px
